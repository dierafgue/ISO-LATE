...
# =============================================================================
    ...
    # -----------------------------------------------------------------
    # DEBUG THA - verificación de señales, tamaños y reconstrucción abs
    # -----------------------------------------------------------------
    with st.expander("🐞 Debug THA - revisión interna", expanded=False):

        M_fix_arr = np.asarray(M_fix, float)
        M_ais_arr = np.asarray(M_ais, float)

        st.markdown("### 1) Dimensiones básicas")
        st.write("dt =", dt)
        st.write("len(ag_filt) =", len(ag))
        st.write("shape(a_t fijo) =", a_fix.shape)
        st.write("shape(M_cond fijo) =", M_fix_arr.shape)
        st.write("shape(a_t aislado) =", a_ais.shape)
        st.write("shape(M_cond aislado) =", M_ais_arr.shape)

        st.markdown("### 2) Verificación de matriz de masas diagonal")
        if M_fix_arr.ndim == 2 and M_fix_arr.shape[0] == M_fix_arr.shape[1]:
            off_fix = M_fix_arr - np.diag(np.diag(M_fix_arr))
            st.write("Máx |offdiag(M_cond fijo)| =", float(np.max(np.abs(off_fix))))
            st.write("diag(M_cond fijo) =", np.round(np.diag(M_fix_arr), 6))
        else:
            st.warning("M_cond fijo no es cuadrada.")

        if M_ais_arr.ndim == 2 and M_ais_arr.shape[0] == M_ais_arr.shape[1]:
            off_ais = M_ais_arr - np.diag(np.diag(M_ais_arr))
            st.write("Máx |offdiag(M_cond aislado)| =", float(np.max(np.abs(off_ais))))
            st.write("diag(M_cond aislado) =", np.round(np.diag(M_ais_arr), 6))
        else:
            st.warning("M_cond aislado no es cuadrada.")

        st.markdown("### 3) Estadísticos del registro y aceleraciones")
        st.write("ag_filt -> min, max =", float(np.min(ag)), float(np.max(ag)))

        for i in range(min(a_fix.shape[0], 5)):
            st.write(
                f"a_t fijo piso/DOF {i+1} -> min, max =",
                float(np.min(a_fix[i, :])),
                float(np.max(a_fix[i, :]))
            )

        for i in range(min(a_ais.shape[0], 5)):
            st.write(
                f"a_t aislado DOF {i+1} -> min, max =",
                float(np.min(a_ais[i, :])),
                float(np.max(a_ais[i, :]))
            )

        st.markdown("### 4) Comparación de aceleración absoluta reconstruida (FIJA)")
        nt_fix = a_fix.shape[1]
        ag_fix_dbg = ag[:nt_fix] if len(ag) >= nt_fix else np.pad(ag, (0, nt_fix - len(ag)))

        a_abs_fix_plus  = a_fix + ag_fix_dbg.reshape(1, -1)
        a_abs_fix_minus = a_fix - ag_fix_dbg.reshape(1, -1)

        for i in range(min(a_fix.shape[0], 3)):
            st.write(
                f"a_abs(+) fijo DOF {i+1} -> min, max =",
                float(np.min(a_abs_fix_plus[i, :])),
                float(np.max(a_abs_fix_plus[i, :]))
            )
            st.write(
                f"a_abs(-) fijo DOF {i+1} -> min, max =",
                float(np.min(a_abs_fix_minus[i, :])),
                float(np.max(a_abs_fix_minus[i, :]))
            )

        st.markdown("### 5) Comparación de cortante basal FIJO con +ag y -ag")
        m_fix_dbg = np.diag(M_fix_arr).reshape(-1, 1)

        F_fix_plus  = m_fix_dbg * a_abs_fix_plus
        F_fix_minus = m_fix_dbg * a_abs_fix_minus

        Vb_fix_plus  = np.sum(F_fix_plus, axis=0)
        Vb_fix_minus = np.sum(F_fix_minus, axis=0)

        st.write(
            "Vb fijo con a_abs = a_rel + ag -> Vmax, Vmin =",
            float(np.max(Vb_fix_plus)),
            float(np.min(Vb_fix_plus))
        )
        st.write(
            "Vb fijo con a_abs = a_rel - ag -> Vmax, Vmin =",
            float(np.max(Vb_fix_minus)),
            float(np.min(Vb_fix_minus))
        )

        st.markdown("### 6) Verificación de longitud temporal")
        st.write("nt fijo =", nt_fix)
        st.write("nt aislado =", a_ais.shape[1])
        st.write("len ag =", len(ag))
        st.write("¿ag y a_t fijo tienen misma longitud?", len(ag) == nt_fix)
        st.write("¿ag y a_t aislado tienen misma longitud?", len(ag) == a_ais.shape[1])

        st.markdown("### 7) Tabla rápida de base shear de prueba (FIJO)")
        df_dbg = pd.DataFrame({
            "Caso": [
                "ETABS aprox desde imagen",
                "ISO-LATE con a_rel + ag",
                "ISO-LATE con a_rel - ag",
            ],
            "Vmax [tonf]": [
                21.1264,
